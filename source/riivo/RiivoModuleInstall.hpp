...
namespace Riivo
{
	...
	struct ModuleParams
	{
		...
		//! RIV1 service. Ignored by the module unless tableKind selects it;
		//! every word is checked at init before a single read is served.
		u32 tableKind;  //!< 0 = whole-file redirect table, 1 = RIV1 manifest
		u32 genBase;    //!< staged generated bytes, PPC view (0 when none)
		u32 genSize;    //!< bytes available at genBase
		u32 declLo;     //!< declared virtual-disc bytes, low word
